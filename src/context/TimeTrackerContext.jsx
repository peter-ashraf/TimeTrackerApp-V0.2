...
export const TimeTrackerProvider = ({ children }) => {
  ...
  const calculateHoursSpentOutside = useCallback(
    (intervals) => {
  ...
  const recalculateEntryFields = useCallback(
    (entry) => {
      ...
      if (hoursWorked === 0 && entry.intervals?.length > 0) {
        const hasValidIntervals = entry.intervals.some(
          (interval) => interval.in && interval.out,
        );
        if (hasValidIntervals) {
          console.warn(
            "[recalculateEntryFields] hours_worked=0 on non-empty intervals:",
            {
              date: entry.date,
      ...
      const isSpecialDay =
        entry.type === "Holiday" || entry.type === "Vacation";
      const useDoubleFactor = isWeekend || isSpecialDay;

      const isHalfDaySpecial =
        entry.duration === 0.5 &&
        (entry.type === "Vacation" ||
          entry.type === "Sick Leave" ||
          entry.type === "To Be Added");

      const isFullDaySpecial =
        entry.duration === 1 &&
        (entry.type === "Vacation" ||
          entry.type === "Sick Leave" ||
          entry.type === "To Be Added");
      ...
      if (isFullDaySpecial) {
        ...
        extraHoursWithFactor =
          extraHours > 0 ? extraHours * factor : extraHours;
      }
  ...
  const checkIn = useCallback(async () => {
    ...
    try {
      ...
      if (todayEntry) {
        ...
        updatedEntry.intervals = [
          ...updatedEntry.intervals,
          { in: timeString, out: null },
        ];
        updatedEntry.lastModified = now.toISOString();

        const saveResult = await timeEntryContext.saveTimeEntriesData(
          updatedEntry,
          showAlert,
        );
        ...
        if (saveResult.success) {
          ...
          if (navigator.onLine)
            supabaseData
              .suppressDailyReminder(currentUser.id, today)
              .catch((e) => console.error("Suppression failed:", e));
  ...
  const calculateOvertimeDetails = useCallback(
    (entries, periodStart, periodEnd) => {
      ...
      periodEntries.forEach((entry) => {
        ...
        const allComplete = entry.intervals.every(
          (interval) => interval.in && interval.out,
        );
        if (!allComplete) {
          return;
  ...
  useEffect(() => {
    ...
    if (!lastBackup) {
      if (timeEntryContext.entries.length > 0) {
        const oldestEntry = [...timeEntryContext.entries].sort((a, b) =>
      ...
      const daysSinceBackup = Math.floor(
        (today - lastBackupDate) / (1000 * 60 * 60 * 24),
      );
  ...
  useEffect(() => {
    if (
      !currentUser ||
      !isContextReady ||
      timeEntryContext.entries.length === 0
    )
      return;
    if (migrationRef.current) return;

    const needsMigration = timeEntryContext.entries.some(
      (e) =>
      ...
      const runMigration = () => {
        const migratedEntries = timeEntryContext.entries.map((entry) => {
          ...
          if (isFullDaySpecial) {
            ...
            extraHours = hoursWorked - halfDayBaseline;
            extraHoursWithFactor =
              extraHours > 0 ? extraHours * 1.5 : extraHours;
          } else if (entry.doubleHours) {
            extraHours = hoursWorked;
            extraHoursWithFactor = hoursWorked * 2;
          } else if (useDoubleFactor && entry.type !== "Regular") {
  ...
  const updateEntry = useCallback(
    async (date, updates) => {
      ...
      try {
        const existingEntry = timeEntryContext.entries.find(
          (e) => e.date === date,
        );
        if (!existingEntry) {
        ...
        const entryToSave = updates.intervals
  ...
  const deleteEntry = useCallback(
    async (date) => {
      ...
      setConfirmModal({
        ...
        onConfirm: async () => {
          try {
            const entryToDelete = timeEntryContext.entries.find(
              (entry) => entry.date === date,
            );

            if (!entryToDelete) {
              console.error(
                "[Delete] Entry not found in local state for date:",
                date,
              );
            ...
            if (navigator.onLine && currentUser && !currentUser.isLocalOnly) {
              try {
                ...
                for (let attempt = 1; attempt <= 2; attempt++) {
                  try {
                    ...
                    if (result?.success) {
                      timeEntryContext.setEntries((prev) =>
                      ...
                      showAlert(
                        "Delete failed because this entry lost its database ID. Please refresh and try again.",
                        "error",
                      );
                      break;
                    } else if (result?.reason === "no_auth_token") {
                      showAlert(
                        "Delete failed: session expired. Please refresh the page and try again.",
                        "error",
                      );
                      break;
                      ...
                      if (attempt < 2) {
                        showAlert(
                          "Delete timed out after tab switch. Retrying automatically...",
                          "warning",
                        );
                        console.warn(
                          "[Delete] Attempt",
                      ...
                      showAlert(
                        "Delete failed. Entry was not removed. Please try again.",
                        "error",
                      );
                      break;
                    }
                ...
                if (deleteSuccess) {
                  showAlert(
                    "Entry deleted successfully from database",
                    "success",
                  );
                } else if (lastError) {
                  console.error(
                    "[Delete] Failed after all attempts for date:",
                    date,
                  );
              ...
              console.warn(
                "[Delete] Online delete unavailable, deleting locally only",
                {
              ...
              if (currentUser) {
                const entriesKey = `timeEntries_${currentUser.id}`;
                const currentEntries =
                  getSimpleEncryptedItem(entriesKey, currentUser.username) ||
                  [];
                setSimpleEncryptedItem(
                  entriesKey,
                  currentEntries.filter((e) => e.date !== date),
              ...
              if (!currentUser) {
                showAlert(
                  "Entry deleted locally, but no active user session was available for cloud sync.",
                  "warning",
                );
              } else if (currentUser.isLocalOnly) {
                showAlert(
                  "Entry deleted locally in local-only mode.",
                  "success",
                );
              } else if (!navigator.onLine) {
                showAlert(
                  "Entry deleted locally while offline. It will sync when connection is restored.",
                  "success",
                );
              } else {
                showAlert(
                  "Entry deleted locally. Cloud delete was unavailable.",
                  "warning",
                );
              }
            }
